...
function compile(data, options = {}) {
  ...
  const filename =
    (options.filename &&
      options.filename.substr(options.filename.lastIndexOf("/") + 1)) ||
    "index.vue";
  ...
  if (style != "") {
    ...
    if (Object.keys(style).length > 0) {
      ...
      if (app.cssVars.length == 0) {
        style =
          style +
          `const __VUE_STYLESHEET__ = __REACT_STYLESHEET__.create(${JSON.stringify(
            styles,
            null,
            2
          )})\n\n`;
  ...
  if (app.script || app.scriptSetup) {
    ...
    if (!app.scriptSetup && app.template) {
      res.script.content = SFC.rewriteDefault(
        res.script.content,
        "__DEFAULT_PRE_TEMPLATE__",
        []
      );
      ...
      template.code = template.code.replace(
        ...
        "function __TEMPLATE_RENDER__"
      );
      res.script.content +=
        "\n\n" +
        template.code +
        "\n\nconst __DEFAULT_WITH_TEMPLATE__ = Object.assign({ render: __TEMPLATE_RENDER__, __name: " +
        JSON.stringify(name);
      if (style) {
