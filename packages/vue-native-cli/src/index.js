...
function getVueNativeDependencyPackageInstallationCommand() {
  ...
  if (isYarnPresent) {
    vueNativePkgInstallationCommand = {
      ...
      optionsArr: [
        "add",
        `${constantObjects.vueNativePackages.vueNativeRuntime}`,
        "--exact"
      ]
    };
  } else {
...
function getVueNativeDevDependencyPackageInstallationCommand() {
  ...
  if (isYarnPresent) {
    vueNativePkgInstallationCommand = {
      ...
      optionsArr: [
        "add",
        `${constantObjects.vueNativePackages.vueNativeCompiler}`,
        '@babel/core@^7.0.0',
        "--exact",
        "--dev"
      ]
...
async function setupVueNativeApp(projectName, cmd, isCrna = false) {
  ...
  const appFileContent = fs.readFileSync(
    path.resolve(__dirname, "./utils/app.js")
  );
  fs.writeFileSync(
    path.join(projectName, constantObjects.appFileName),
    appFileContent
  );

  const mainVueFileContent = fs.readFileSync(
    path.resolve(__dirname, "./utils/main.vue")
  );
  fs.writeFileSync(
    path.join(projectName, constantObjects.mainVueFileName),
    mainVueFileContent
  );
